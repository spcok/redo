  ...
  return useMutation({
    ...
    onMutate: async (val: AnimalPayload) => {
      ...
      await db.query(
        `INSERT INTO animals (
          ...
          origin, is_boarding, is_quarantine, created_by, modified_by
        ...
        [
          ...
          val.origin || null, val.is_boarding, val.is_quarantine, val.currentUserId
        ]
      );
    },
    mutationFn: async (val: AnimalPayload) => {
      ...
      const { error } = await supabase.from('animals').insert({
        ...supabasePayload, id: animalId, created_by: currentUserId, modified_by: currentUserId
      });
    ...
    onMutate: async (val: AnimalPayload) => {
      ...
      await db.query(
        `UPDATE animals SET 
          ...
          origin=$30, is_boarding=$31, is_quarantine=$32, modified_by=$33, updated_at=now()
         WHERE id = $1`,
         [
            val.animalId, val.entity_type, val.parent_mob_id || null, val.census_count, val.name || null, val.species || null, val.latin_name || null, val.category || null, val.location || null,
            val.hazard_rating || null, val.is_venomous, val.weight_unit, val.average_target_weight || null, val.date_of_birth || null, val.is_dob_unknown,
